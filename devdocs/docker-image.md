...
- Runs the `gunicorn -w $NUM_PROCESSES -b :$DRIVER_PORT "netconfdriver:create_wsgi_app()"` command to start the driver application with a Gunicorn container (standard for Python production applications)
...
rm -rf ./docker/whls
mkdir ./docker/whls
cp dist/netconfdriver-<release version number>-py3-none-any.whl docker/whls/
```
...
```
cd docker
docker build -t netconf-driver:<release version number> .
```